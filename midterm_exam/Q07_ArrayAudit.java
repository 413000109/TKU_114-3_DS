public class Q07_ArrayAudit {
    ...
    public static void main(String[] args) {
        ...
        System.out.print("清理後資料："); printArray(cleaned);
        System.out.print("原始資料："); printArray(readings);
    }
    ...
    public static int countValid(int[] data) {
        int count = 0;
        for (int v : data) if (isValid(v)) count++;
        return count;
    }
    ...
    public static int[] createCleanCopy(int[] data) {
        ...
        for (int i = 0; i < data.length; i++) {
            clean[i] = isValid(data[i]) ? data[i] : INVALID_MARK;
        }
        return clean;
    }
